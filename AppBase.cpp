...
namespace EngineCore
{
	...
	bool AppBase::CreateVertexShader()
	{
	...
	bool AppBase::CreatePixelShader()
	{
	...
	bool AppBase::CreatePipelineStateObject()
	{
		// create input layout
	...
	bool AppBase::CreateVertexBuffer()
	{
		...
		if (FAILED(hr))
		{
			OutputDebugString(L"Failed to create upload heap for vertex buffer.\n");
			return false;
		}
	...
	bool AppBase::CreateIndexBuffer()
	{
		// Create index buffer
		...
		if (FAILED(hr))
		{
			OutputDebugString(L"Failed to create default heap for index buffer.\n");
			return false;
		}
		...
		if (FAILED(hr))
		{
			OutputDebugString(L"Failed to create upload heap for index buffer.\n");
			return false;
		}
	...
	bool AppBase::CreateDepthStencilBuffer()
	{
		HRESULT hr;
	...
	bool AppBase::CreateConstantBuffer()
	{
		HRESULT hr;
	...
	DXGI_FORMAT AppBase::GetDXGIFormatFromWICFormat(WICPixelFormatGUID& wicFormatGUID)
	{
	...
	WICPixelFormatGUID AppBase::GetConvertToWICFormat(WICPixelFormatGUID& wicFormatGUID)
	{
	...
	int AppBase::GetDXGIFormatBitsPerPixel(DXGI_FORMAT& dxgiFormat)
	{
	...
	int AppBase::LoadImageDataFromFile(BYTE** imageData, D3D12_RESOURCE_DESC& resourceDescription, LPCWSTR filename, int& bytesPerRow)
	{
		HRESULT hr;
